...
ap.add_argument("-f", "--file", default='image/567.json', help="Path to the analyzed colour group.json")
...
def perform_action(board_state, column_fm, column_to):
    ...
    if len(board_state[column_to]) > 0 and empty_space < height:
        ...
        if len(board_state[column_fm]) > empty_space:
            if board_state[column_fm][-empty_space] == target_color:
                return None
...
def find_possibilities(board_state):
    ...
    for i in range(0, columns):
        ...
        if column_complete(board_state[i]):
            continue
